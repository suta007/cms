...
@section('content')
	...
	<x-div class="mx-auto px-5 col-12">
		<div class="mx-auto col-xl-6 col-md-8">
			...
			<form method="POST" action="{{ route('admin.menu.store') }}" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
				@csrf

				<div class="row mb-3">
					<label for="ordering" class="col-sm-4 col-form-label text-md-end form-required">ลำดับ</label>
					<div class="col-sm-8">
						<input type="number" name="ordering" id="ordering" value="{{ old('ordering') }}" class="form-control @error('ordering') is-invalid @enderror" required min="1" step="1">
						@error('ordering')
				...
				<div class="row mb-3">
					...
					<div class="col-sm-8">
						<input type="text" name="route" id="route" value="{{ old('route') }}" class="form-control @error('route') is-invalid @enderror">
						@error('route')
							<span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
						@enderror
					</div>
				...
				<div class="row mb-3">
					...
					<div class="col-sm-8">
						<select name="guard" id="guard" class="form-select @error('guard') is-invalid @enderror">
							<option value="" @selected(old('guard') == '')>ทั้งหมด</option>
							<option value="admin" @selected(old('guard') == 'admin')>ผู้ดูแลระบบ</option>
							<option value="user" @selected(old('guard') == 'user')>ผูัใช้งาน</option>
						</select>
						@error('guard')
							<span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
						@enderror
