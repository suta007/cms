...
@section('content')
	<x-div class="mx-auto fw-bold fs-5 mb-2 col-12 col-xl-8 text-web">
		Setup
		<a href="{{ route('admin.setup.create') }}" class="btn btn-web float-end mb-2">
			<i class="fa fa-plus me-2" aria-hidden="true"></i> เพิ่มSetup
		</a>
	</x-div>
	<div class="clearfix"></div>
	<x-div class="mx-auto px-5 col-12 col-xl-8">
		<form method="GET" action="{{ route('admin.setup.index') }}" accept-charset="UTF-8" class="form-inline my-2 my-lg-0 float-start me-2" role="perPage">
		...
		<form method="GET" action="{{ route('admin.setup.index') }}" accept-charset="UTF-8" class="form-inline my-2 my-lg-0 float-end" role="search">
			<div class="input-group">
				...
				@foreach ($datas as $item)
					<tr>
						...
						<td class="text-nowrap" style="width: 1%;">
							<a href="{{ route('admin.setup.show', $item->id) }}" data-tooltip="ดู{{ $item->name }}" class="btn btn-success btn-sm"><i class="fa-solid fa-eye"></i></a>
							<a href="{{ route('admin.setup.edit', $item->id) }}" data-tooltip="แก้ไข{{ $item->name }}" class="btn btn-primary btn-sm"><i class="fa-solid fa-pen-to-square"></i></a>

							<form method="POST" action="{{ route('admin.setup.destroy', $item->id) }}" accept-charset="UTF-8" style="display:inline">
								@method('DELETE')
								@csrf
